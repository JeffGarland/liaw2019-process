...
std::process proc1{"/bin/bash", {}, std::process_env{{"VARIABLE", "42"}, {"VAR", "12"}}};
std::process proc2{"/bin/bash", {}, std::process_env{{"VARIABLE", {"42"}}, {"PATH", {"/usr/bin", "/bin"}}}};
...
// This class is just for readability
template<typename T>
concept convertible-to-string-view = 
...
// 
template<typename T>
    requires convertible-to-string-view<T> || 
        (requires (T t)
        {
            {tuple_size<T>{}} -> convertible_to<integral_constant<size_t,2u>>;
            {get<0>(t)} -> convertible-to-string-view;
            {get<1>(t)} -> convertible-to-string-view;
        } || 
        requires (T t)
        {
            {tuple_size<T>{}} -> convertible_to<integral_constant<size_t,2u>>;
            ...
            {*ranges::begin(get<1>(t))} -> convertible-to-string-view;
        })
class process_env {
public:
