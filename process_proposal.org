...
It would be possible to accomodate all those scenarios with complex overloads and helper functions. 
However, the only real platform-specific is the seperator in a list, while everything else is a standard operation.

This the `process_env` shall have this character as `static` member, while `process_env` only accepts ranges of `string_view`. 
With that a user can build his own lists like so:


#+BEGIN_SRC c++

std::vector<std::string> list_env;
list_env.push_back("VARIABLE=42");

std::string path = "PATH=/bin";

std::vector<std::string> paths = {"/usr/bin", "/opt/bin"};
for (auto p : paths)
  path += std::process_env<std::string>::list_seperator + p;

list_env.push_back(path);

std::process proc{"/bin/bash", {}, std::process_env{list_env}};

#+END_SRC

With an environment library like P1275R0 it could be use like this, provided that the `value_type` of the container is convertible to a `string_view`.

#+BEGIN_SRC
std::process proc{"/bin/bash", {}, std::process_env{std::environment}};
#+END_SRC

Similarly the constructors shall accept iterator pairs.

For convenience there will also be overloads of `initializer_list` so that the following usage is also possible. 

#+BEGIN_SRC c++

std::process proc{"/bin/bash", {}, std::process_env{"VARIABLE=42", "VAR1=12"}};
...
template<typename T>
    requires         
      ...
      convertible_to<T,   wstring_view> ||
      convertible_to<T,  u8string_view> ||
      convertible_to<T, u16string_view> ||
      convertible_to<T, u32string_view> 
class process_env {
public:
    using char_type = deduced-from-T;
    constexpr static char_type list_seperator = implementation-defined;
